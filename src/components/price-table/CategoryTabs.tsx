...
export function CategoryTabs({
  ...
  return (
    <div className="relative flex items-center mb-4">
      {showNavButtons && tabsOffset > 0 && (
        <Button
          variant="outline"
          size="icon"
          className="absolute left-0 z-10 mr-1 h-7 w-7 rounded-full bg-background shadow-sm border-border"
          onClick={handlePrev}
        >
      ...
      <div className={cn(
        "overflow-hidden flex-grow px-4",
        showNavButtons && tabsOffset > 0 && "ml-4",
        showNavButtons && tabsOffset < maxOffset && "mr-4"
      )}>
        <TabsList className="flex w-full justify-start overflow-x-auto scrollbar-none">
          {visibleCategories.map(category => (
            <div key={category.id} className="flex items-center">
              <TabsTrigger
                value={category.id}
                className="relative whitespace-nowrap py-0 my-0 mx-0 text-sm font-thin text-left rounded-none px-[7px]"
              >
                {category.name}
                ...
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-6 w-6 p-0 ml-1 rounded-full opacity-60 hover:opacity-100"
                    >
      ...
      {showNavButtons && tabsOffset < maxOffset && (
        <Button
          variant="outline"
          size="icon"
          className="absolute right-0 z-10 ml-1 h-7 w-7 rounded-full bg-background shadow-sm border-border"
          onClick={handleNext}
        >
