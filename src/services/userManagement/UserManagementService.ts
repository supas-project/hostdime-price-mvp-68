
import { Session } from '@supabase/supabase-js';

export interface UserProfile {
  id: string;
  email: string;
  created_at: string;
  user_metadata: {
    nome_completo?: string;
    tipo?: 'user' | 'admin';
  };
}

export interface CreateUserData {
  email: string;
  password: string;
  nome_completo: string;
  tipo: 'user' | 'admin';
}

export interface UpdateUserData {
  email: string;
  nome_completo: string;
  tipo: 'user' | 'admin';
}

/**
 * Unified user management service
 * Replaces duplicated UserService and UserAdminService
 */
export class UserManagementService {
  private static readonly FUNCTION_URL = 'https://nglwjdpocxelvarqjgts.supabase.co/functions/v1/user-admin';
  private static readonly SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im5nbHdqZHBvY3hlbHZhcnFqZ3RzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDU4NTE3OTMsImV4cCI6MjA2MTQyNzc5M30.8xCetXorVi2SehrE_Tfgf-I_96o75alWXTMSHZLNh7s';

  private static getHeaders(session: Session) {
    return {
      'Authorization': `Bearer ${session.access_token}`,
      'apikey': this.SUPABASE_ANON_KEY,
      'Content-Type': 'application/json',
      'x-client-info': 'supabase-js-web/2.49.8'
    };
  }

  static async listUsers(session: Session): Promise<UserProfile[]> {
    console.log('📋 UserManagement: Listing users');
    
    try {
      const response = await fetch(this.FUNCTION_URL, {
        method: 'GET',
        headers: this.getHeaders(session)
      });

      if (!response.ok) {
        const errorText = await response.text();
        console.error('❌ UserManagement: HTTP error:', response.status, errorText);
        throw new Error(`HTTP ${response.status}: ${errorText}`);
      }

      const data = await response.json();
      console.log('✅ UserManagement: Users loaded:', data?.users?.length || 0);
      return data?.users || [];
    } catch (error) {
      console.error('❌ UserManagement: Error listing users:', error);
      if (error instanceof Error) {
        throw new Error(`Failed to list users: ${error.message}`);
      }
      throw new Error('Failed to list users: Unknown error');
    }
  }

  static async createUser(session: Session, userData: CreateUserData): Promise<void> {
    console.log('➕ UserManagement: Creating user:', userData.email);
    
    try {
      const response = await fetch(this.FUNCTION_URL, {
        method: 'POST',
        headers: this.getHeaders(session),
        body: JSON.stringify(userData)
      });

      if (!response.ok) {
        const errorText = await response.text();
        console.error('❌ UserManagement: HTTP error:', response.status, errorText);
        throw new Error(`HTTP ${response.status}: ${errorText}`);
      }

      console.log('✅ UserManagement: User created successfully');
    } catch (error) {
      console.error('❌ UserManagement: Error creating user:', error);
      if (error instanceof Error) {
        throw new Error(`Failed to create user: ${error.message}`);
      }
      throw new Error('Failed to create user: Unknown error');
    }
  }

  static async updateUser(session: Session, userId: string, userData: UpdateUserData): Promise<void> {
    console.log('✏️ UserManagement: Updating user:', userId);
    
    try {
      const response = await fetch(this.FUNCTION_URL, {
        method: 'PUT',
        headers: this.getHeaders(session),
        body: JSON.stringify({ userId, ...userData })
      });

      if (!response.ok) {
        const errorText = await response.text();
        console.error('❌ UserManagement: HTTP error:', response.status, errorText);
        throw new Error(`HTTP ${response.status}: ${errorText}`);
      }

      console.log('✅ UserManagement: User updated successfully');
    } catch (error) {
      console.error('❌ UserManagement: Error updating user:', error);
      if (error instanceof Error) {
        throw new Error(`Failed to update user: ${error.message}`);
      }
      throw new Error('Failed to update user: Unknown error');
    }
  }

  static async deleteUser(session: Session, userId: string): Promise<void> {
    console.log('🗑️ UserManagement: Deleting user:', userId);
    
    try {
      const response = await fetch(this.FUNCTION_URL, {
        method: 'DELETE',
        headers: this.getHeaders(session),
        body: JSON.stringify({ userId })
      });

      if (!response.ok) {
        const errorText = await response.text();
        console.error('❌ UserManagement: HTTP error:', response.status, errorText);
        throw new Error(`HTTP ${response.status}: ${errorText}`);
      }

      console.log('✅ UserManagement: User deleted successfully');
    } catch (error) {
      console.error('❌ UserManagement: Error deleting user:', error);
      if (error instanceof Error) {
        throw new Error(`Failed to delete user: ${error.message}`);
      }
      throw new Error('Failed to delete user: Unknown error');
    }
  }

  static async sendPasswordReset(email: string): Promise<void> {
    const { supabase } = await import('@/lib/supabaseClient');
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${window.location.origin}/auth/reset-password`,
    });
    
    if (error) throw error;
  }
}
