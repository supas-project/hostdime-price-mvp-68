...
export async function syncDiskDataWithPriceService(): Promise<void> {
  try {
    ...
    const { data: session, error } = await PriceService.supabase.auth.getSession();
    if (error) {
    ...
    try {
      ...
      console.error("Error processing disk data:", innerError);
      // Don't show toast for authentication errors - these are expected for non-logged-in users
      if (innerError instanceof Error && !innerError.message.includes("Authentication")) {
...
export async function initExternalStorageData(): Promise<void> {
  try {
    ...
    try {
      ...
      console.error("Error processing external storage data:", innerError);
      // Don't show toast for authentication errors - these are expected for non-logged-in users
      if (innerError instanceof Error && !innerError.message.includes("Authentication")) {
